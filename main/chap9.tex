...
\begin{corollary}
    Let \( d\geq 42 \) and \( \mathbf{w} \in \mathbb{Z}^{V_d} \) be a valid configuration with positive support size five. Assume \( (i,x), (i,y), (i',z) \in \mathrm{supp}(\mathbf{w}) \) with \( i < i' \). If \( \mathrm{relcoord}(x) \in \left\{ 0,1,2,3 \right\} \), \( \mathrm{relcoord}(y) \in \left\{ d-6, \dots, d \right\} \) and \( \mathrm{relcoord}(z) \neq M  \), then \( \mathbf{w} \) is not an outcome.
\end{corollary}
...
These two corollaries allow us to implement the following algorithm to exclude certain cases of \( \mathbf{s}' \in \Lambda \).
\begin{algorithm}[H]
\caption{Divide}
\label{alg:divide}
\begin{algorithmic}[1]
...
\begin{proposition}
    ...
    \begin{enumerate}
        ...
        \end{enumerate}
        \item Let \( a = d-6, \dots, d \). Then, \( \mathrm{relcoord}( \frac{a + b - 1}{2}) = M \).
    \end{enumerate}
\end{proposition}
...
\Function{\texttt{conquer}}{$\texttt{relset}$}
    \State \texttt{relset} $\gets \texttt{sorted(relset, key=lambda x: col(x))}$
    \State $\texttt{length} \gets |\texttt{relset}|$
    \If{$\texttt{length} \leq 2$}
        \State \Return \texttt{True} \Comment{Invertibility criterion holds}
    \ElsIf{$\texttt{length} = 3$}
        ...
        \If{$\texttt{same\_column}$} \Comment{Proposition 5.7c}
            \State \Return \texttt{True}
        \ElsIf{$\texttt{col}(x) = \texttt{col}(y)$ \textbf{and} $\texttt{succ}(\texttt{col}(x)) = \texttt{col}(z)$}
            \If{$\texttt{row}(z) \notin \texttt{midpoint}(\texttt{row}(x), \texttt{row}(y))$} \Comment{Proposition 5.7d}
                \State \Return \texttt{True}
            \EndIf
        \EndIf
