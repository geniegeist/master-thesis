...
It remains to show the other 2289 cases of \( \mathbf{s}' \in \Lambda \) with positive support size five. From now on, assume that \( \mathbf{s}' \in \Lambda \) has positive support size five. We will show that all these cases are invalid. 
...
It is our goal to exclude all \( 2289 \) cases of \( \mathbf{s}' \in \Lambda \) with positive support size five. We will make use of the Invertibility Criterion. For that we introduce \emph{relative coordinates}  defined below.
...
\begin{example}
    Let \( d \geq 42 \). Let \( \mathbf{w} \in \mathbb{Z}^{V_d} \) be some valid configuration with support size seven. Now, assume that the relative support set of \( \mathbf{w} \) is 
    \begin{align*}
        \mathrm{relset}(\mathbf{w}) = \left\{ (0,0), (0,d), (1,3), (M,2), (M, d-6), (d-5, M), (d,0) \right\}.
    \end{align*}
...
0     X   .   .   .   .   .   .   .   .   .   .   .   .   .   .   .   .   .   X
    ...
    We see that 
    \begin{align*}
        \mathrm{supp}(\mathbf{w}) = \left\{ (0,0), (0,d), (1,3) \right\} \cup \left\{ (i,2), (j,d-6 ) \right\} \cup \left\{ (d-5,k ) \right\} \cup \left\{ (d,0) \right\}.
    \end{align*}
    for \( i,j,k  = 4, \dots, d-7 \). When \( i = j \), we can apply the Invertibility Criterion with \( \lambda = (3,1, \dots,1, 2, 1, \dots, 1) \) to see that \( \mathbf{w} = \mathbf{0} \), which is a contradiction. So assume \( i \neq j \). Then, we use \( \lambda = (3, 1, \dots, 1) \) to see that \( \mathbf{w} = \mathbf{0} \), which is a contradiction. Hence, \( \mathbf{w} \) cannot be an outcome.
\end{example}