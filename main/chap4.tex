...
\begin{theorem*}
    The upper bound \( \mathrm{deg}(\mathbf w) \leq 2 \cdot |\mathrm{supp}^+(\mathbf w)| - 3 \)
    holds for valid integral outcomes \( \mathbf w \) with \( |\mathrm{supp}^+(\mathbf w)| \leq 5 \). 
\end{theorem*}

From now on, valid outcomes \( \mathbf{w} \) refer to \emph{integral} valid outcomes in \( \mathbb{Z}^{V_d} \) for \emph{finite} \( d \in \mathbb{N} \). To show the theorem, we will first study the supports of valid outcomes; knowing that some kinds of supports cannot be the supports of valid outcomes will help us to prove the theorem. For instance, integer configurations that have support in the entries below marked with an \texttt{*} cannot be valid outcomes as we will see.
\begin{verbatim}
    ·  
    ·  · 
    *  ·  · 
...
Let \( d \in \mathbb{N} \).
The most important tools in the study of outcomes is the \emph{Invertibility Criterion} first introduced in \cite{bik2022classifying}. By Theorem \ref{thm:pascal-outcome} we can characterize outcomes as the roots of Pascal forms on \( \mathbb{Z}^{V_d} \). In the previous chapter, we have already found two bases for the space of Pascal forms: \((\mathrm{row}(0), \dots, \mathrm{row}(d)) \) and \((\mathrm{col}(0), \dots, \mathrm{col}(d)) \). Let us introduce another basis.
...
\begin{example}
    ...
    Now, assume \( \mathbf{w} \) is an outcome with support in \( S \). Since it is an outcome, we have \( \mathrm{diag}(k)(\mathbf{w}) = 0 \) for all \( k = 0, 1,2,3 \). Thus, we have \(  A^{(d)}_{E,S} \mathbf w = \mathbf 0 \).
...
\begin{example}
    We continue Example \ref{ex:decomposition-nsjkfnje}. With \( \lambda = (2,1,1,1,1,1) \) we obtain the following decomposition and pairing matrix
    \begin{gather*}
...
\begin{proof}
    ...
    Let \( \mathbf{w} \) be a valid outcome. Then, \( \mathrm{diag}(k)(\mathbf{w}) = 0 \) for all \( k = 0, \dots, d \). Thus, for all \( k = 0, \dots, d \) we have
    \begin{align*}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \end{bmatrix}.
    \end{align*}
    We substitute \( x = d-i, y = d-j \), and \( z = d-k \).
    Then, we see
    \begin{align*}
        \begin{bmatrix}
            1 & 0 & 0 \\
...
\begin{align*}
    S \subset \left\{ (i,j) \in V_d \mid i < \lvert S \rvert \right\} \quad \text {and} \quad E = \left\{ 0,1, \dots, \lvert S \rvert - 1 \right\}
\end{align*}
