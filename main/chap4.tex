...
\begin{example}
    Fix the degree \( d=6 \). Assume we have some outcome \( \mathbf w \in \mathbb{Z}^{V_6} \) with support in the positions marked with an \texttt{*} below.
    \begin{verbatim}
        · 
        · · 
        * · · 
...
\begin{align*}
    ...
    \end{cases}
\end{align*}
We see that \( \lvert E_i \rvert = \lvert S_i \rvert \) for all \( i = 1, \dots, l \). Define \( E \coloneqq E_1 \cup \dots \cup E_l \).